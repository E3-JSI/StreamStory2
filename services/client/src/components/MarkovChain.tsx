...
import { createSVG, getSVG, TRANSITION_PROPS, createLinearScale, createNodes, createLinks, createMarkers, LinkType, createGraphContainer, getGraphContainer, createPowScale, findMinMaxValues, isValidProb, uniqueId, createStateLinks, createDictId, createStatesDict, createGraphData } from "../utils/markovChainUtils";
...
const MarkovChain = ({ model, onStateSelected }: ModelVisualizationProps) => {
    ...
    useEffect(() => {
        if ((pThreshold >= 0) && (currentScaleIx >= 0) && model.model.scales && model.model.scales.length && data && data.length) {
            const graphData = createGraphData(model.model.scales, statesDictTmp, dictIdTmp, pThreshold);
            setData(graphData)
    ...
    function renderMarkovChain(graphData: any): void {
        ...
        if (graphData && model.model.scales[currentScaleIx]) {
            ...
            createNodes(graphData[currentScaleIx], gNodes, gLinks, gMarkers, x, y, r, color, TRANSITION_PROPS, (a: any, b: any) => {
                const selectedState = model.model.scales[currentScaleIx].states.find((state: any) => state.stateNo === b);
